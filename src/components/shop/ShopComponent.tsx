...
import { FaAnglesRight } from "react-icons/fa6"; // ← اصلاح شد
...
export default function ShopComponent() {
  ...
  const getCatProducts = async (id: number) => {
    try {
      const cat_products = await axios.get(
        `http://realalisaberi.ir/products/category/${id}`
      );
      ...
      <div className="w-full my-10 grid grid-cols-4 gap-10 min-h-screen">
        <div className="col-span-1">
          ...
          <Filter title="Categories" active={!!category_id}>
            <Link
              ...
              className={`w-full flex items-center py-3 border-b cursor-pointer text-gray-500 border-gray-100 hover:text-violet-600 ${
                !category_id ? "text-violet-600 font-bold" : ""
              }`}
            >
              All Products
         ...
         to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg
          shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 font-medium rounded text-sm px-4 py-4 text-center"
                >
                  Clear
                </button>
                <span className="text-sm text-violet-600 font-bold">
